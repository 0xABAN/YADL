...
export default function Auth() {
  ...
  const go = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setErr(null);
    try {
      const r = await apiResult(login ? "/auth/login" : "/auth/signup", {
        method: "POST",
      ...
      if (!r.ok) {
        if (r.status === 409) setErr("Email already registered.");
        else setErr(login ? "Wrong email or password." : "Could not sign up.");
        return;
      }
      router.push("/create");
    } finally {
